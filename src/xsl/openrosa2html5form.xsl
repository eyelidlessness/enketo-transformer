...
<xsl:stylesheet
    ...
    <xsl:key name="bindings-by-id" match="/h:html/h:head/xf:model/xf:bind" use="@id" />
    <xsl:key name="nodeset-bindings" match="/h:html/h:head/xf:model/xf:bind" use="@nodeset" />
    <xsl:key name="preload-bindings" match="/h:html/h:head/xf:model/xf:bind" use="@jr:preload != ''" />
    <xsl:key name="calculate-bindings" match="/h:html/h:head/xf:model/xf:bind" use="@calculate != ''" />
    <xsl:key name="instances" match="/h:html/h:head/xf:model/xf:instance" use="true()" />
    <xsl:key name="model-actions" match="/h:html/h:head/xf:model/xf:setvalue[@event] | /h:html/h:head/xf:model/odk:setgeopoint[@event] " use="local-name()" />
    <xsl:key name="fields-by-ref" match="/h:html/h:body//xf:input | /h:html/h:body//xf:upload | /h:html/h:body//xf:select | /h:html/h:body//xf:select1" use="@ref" />
    <xsl:key name="translations" match="/h:html/h:head/xf:model/xf:itext/xf:translation" use="true()" />
    <xsl:key name="itexts-by-id" match="/h:html/h:head/xf:model/xf:itext/xf:translation/xf:text" use="@id" />
    ...
    <xsl:variable name="default-lang">
        <xsl:choose>
            <xsl:when test="key('translations', true())[@default]/@lang">
                <xsl:value-of select="key('translations', true())[@default]/@lang" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="''" />
            </xsl:otherwise>
            ...
            <root>
                <form autocomplete="off" novalidate="novalidate" class="clearfix" dir="ltr">
                    ...
                    <xsl:attribute name="data-form-id">
                        <xsl:choose>
                            <xsl:when test="key('instances', true())[1]/child::node()/@id">
                                <xsl:value-of select="key('instances', true())/child::node()/@id" />
                            </xsl:when>
                ...
                -->
                    ...
                    <xsl:if test="key('model-actions', 'setgeopoint')">
                        <fieldset id="or-setgeopoint-items" style="display:none;">
                            <xsl:apply-templates select="key('model-actions', 'setgeopoint')[@event]" />
                        </fieldset>
                    </xsl:if>
                    <!--
    ...
    -->
        ...
        <xsl:if test="not( local-name() = 'bind' and count(
            key('fields-by-ref', $nodeset) ) > 0 )">
            <xsl:choose>
    ...
    <xsl:template name="translations">
        ...
        <xsl:for-each select="key('itexts-by-id', $id)">
        ...
        <xsl:variable name="nodeset_a">
            <xsl:choose>
                <xsl:when test="not(substring($nodeset_u, 1, 1) = '/')">
                    <xsl:value-of select="concat('/', local-name(key('instances', true())/child::*[1]), '/', $nodeset_u)"/>
